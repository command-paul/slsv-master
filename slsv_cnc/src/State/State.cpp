...
uint64_t riscv::get_register(uint32_t address){
	...
	if(address < 32) return HART_Vec[HART].GPR[address]; // GPR
	else if(address == 32) return HART_Vec[HART].PC; // PC
	else if(address < 65) return HART_Vec[HART].FPR[address-32]; // FPR
	else {
